...
//                飞机模式才更新灰色线条
    ...
    private void setLineProgress(float start, float end, boolean isLightPoints) {       //对一条线的进程进行更新设置
        if (mKeyframes == null)
        ...
        if (isLightPoints) {
            ...
            mLightPoints = mKeyframes.getRangeValue(start, end);            //获取到当前进程下所在keyfrmaes中的点集合
    ...
    private void startDraw(Canvas canvas) {             //进行绘画的主函数
        ...
        if (mDarkPoints != null) {
            ...
            canvas.drawPoints(mDarkPoints, mPaint);           //拓展增加需要绘制的点集合
        }
        mDarkLineSemaphore.release();
        try {
            mLightLineSemaphore.acquire();
    ...
    //路径的关键帧管理类 用来提供在一个进程范围内的线段上的所有点集合
